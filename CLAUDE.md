...
- **CLI gallery state stays on the filesystem.** This app is a companion UI for `@fal-ai/genmedia-cli` (source: `/Users/tom/code/fal-ai/genmedia-cli`). Generation history is owned by the CLI in `~/.genmedia/gallery/sessions/<session_id>/data.json` (schema: `SessionPayload` in the CLI's `src/lib/gallery-template.ts`) — treat it as read-only and re-read the whole file on change (it is rewritten, not appended). Legacy project intent lives in a `storyboard.json` shared with the Claude Code agent driving the CLI; write it atomically (temp file + rename). Do not introduce a database to _mirror_ that file-owned state.
- **Story engine state lives in SQLite, one DB per project folder** (`<projectDir>/story.db`, next to that project's `storyboard.json`; drizzle-orm on the `node:sqlite` driver; gitignored; `STORY_DB_PATH` override for tests). It owns sequences/scenes/shots/frames plus characters/locations/elements — see `docs/PLAN-STORY.md`. All access goes through `src/db/`; skills write via `scripts/story.ts` (`--project <dir>` or cwd).
